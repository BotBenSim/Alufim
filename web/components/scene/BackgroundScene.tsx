...
export function BackgroundScene({ mode = "fixed", className }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="rainbow absolute left-1/2 top-[-14%] aspect-[2/1] w-[min(150vw,1100px)] -translate-x-1/2 bg-[radial-gradient(circle_at_50%_100%,rgba(255,255,255,0)_0_32%,#FF8787_32%_38%,#FFC078_38%_44%,#FFE066_44%_50%,#8CE99A_50%_56%,#74C0FC_56%_62%,#B197FC_62%_68%,rgba(255,255,255,0)_68%_100%)] opacity-80 [mask-image:linear-gradient(180deg,#000_60%,transparent_100%)]" />
      {clouds.map((cls) => (
        <div key={cls} className={`skyCloud ${cls}`} />
      ))}
      <div className="ground absolute bottom-0 left-0 right-0 h-[20vh] min-h-[120px] bg-gradient-to-b from-[#8FD957] via-[#6FC23E] to-[#57AC2C]">
